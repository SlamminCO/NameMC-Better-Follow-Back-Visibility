    ...
    if (sp2_1 && sp2) {
        ...
        for (let user of list) {
            if (user.innerHTML.search("bold") == -1) {
                ...
                let buttons = user.getElementsByClassName("btn btn-block btn-sm btn-success")
                buttons[0].setAttribute("class", "btn btn-block btn-sm btn-success btn-not-follower")
                buttons[0].setAttribute("id", "UnfollowButton")
                
                if (autoUnfollow) {
                    document.getElementById("UnfollowButton").click()
                }
            } else {
                let sp1 = document.createElement("td")
